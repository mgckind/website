...
def get_authors(entry):
    ...
    for a in authors:
        ...
        if last == 'Kind':
            ...
            temp['main']=False
            temp['color']='blue'
        if i < num_authors: shortlist.append(first+' '+last)
        newlist.append(first+' '+last)
        new_dict.append(temp)

    if i >=num_authors : shortlist.append('et al.')
...
with open(os.path.join(ROOT,'Research/website/publications.json'),'w') as out:
    print('ddd')
    Alldata=[]
    Alldatamine=[]
    datamine=[]
...
with open(os.path.join(ROOT,'Research/website/publications_sel.json'),'w') as out:
